<?php
  ...
  //use more ternary operators to shorten if's

  // Admins can't delete themselves
  if (ADMIN && $user['id'] === $_SESSION['user']['id']) redirect_with_errors(base_path . "/users/show.php?id={$user['id']}", "Admin's are not allowed to delete themself!");
  ...
  if ($user['id'] === $_SESSION['user']['id']) {
    unset($_SESSION['user']);
    redirect_with_success(base_path . "/", "You have successfully deleted your Lokham account.");
  }

  // If the administrator deleted a user let them know they were deleted successfully
  redirect_with_success(base_path . "/users", "You have successfully deleted user - " . $user['first_name'] . " " . $user['last_name']);